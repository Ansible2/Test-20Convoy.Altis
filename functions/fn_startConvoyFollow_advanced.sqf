...
[
	{
		...
		_args params ["_vics","_leadVic"];
		{
			...
			// if (_vehicleAheadHasNotMovedEnough) then {break};

			private _currentVehiclePosition = getPosATLVisual _currentVehicle;
			private _deleteStartIndex = -1;
			private _numberToDelete = 0;
			{
				hint str [(_currentVehiclePosition distance _x), count _currentVehicleDrivePath];
			...
			private _debugObject = "Sign_Arrow_Large_Cyan_F" createVehicle _vehicleAheadPosition;
